...
Page({
  ...
  save: function() {
    ...
    if (this.data.ifAddPage) {
      // 如果是添加页面
      var tmp_start_date =
        this.data.startDate === "选择日期"
          ? app.globalData.date
          : this.data.startDate;
      var tmp_start_concret_time =
        this.data.startTime === "选择时间"
          ? time.getConcreteTime(new Date().getHours(), new Date().getMinutes())
          : this.data.startTime;
      var tmp_end_date =
        this.data.endDate === "选择日期"
          ? app.globalData.date
          : this.data.endDate;
      var tmp_end_concret_time =
        this.data.endTime === "选择时间"
          ? time.getConcreteTime(new Date().getHours(), new Date().getMinutes())
          : this.data.endTime;
      this.setData({
      ...
      wrapper
        .wxRequestWrapper(api.addScheduleitem, "POST", {
        ...
        .then(data => {
          ...
          console.log(
            "得到服务器返回的当天scheduleitems",
            newScheduleitmesForDay
          );
          ...
          app.globalData.scheduleItems = schedule.warpScheduleItems(
            newScheduleitmesForDay, app.globalData.showCompletedSchedule
          );
          console.log(app.globalData.scheduleItems);
          app.globalData.ifAddSchedule = true;
          app.globalData.newItemDate = this.data.startDate;
          app.globalData.ifSameDay =
            app.globalData.date == this.data.startDate.slice(0, 10);
          ...
          this.setData({ ifPressAdd: false }); // 设置防止多次点击发送请求的flag
        })
        .catch(errno => {
      ...
      wrapper
        .wxRequestWrapper(
          api.updateScheduleitem +
            app.globalData.scheduleItems[this.data.itemIndex].scheduleItem_id,
          "PUT",
          {
            //在这里把修改传的参数写上
            start_time: tmp_start_time,
        ...
        .then(data => {
          ...
          console.log(data);
          app.globalData.scheduleItems = schedule.warpScheduleItems(
            data["scheduleItems"], app.globalData.showCompletedSchedule
          ); // ?
          ...
          app.globalData.ifChangeScheduleStartDate = !(
